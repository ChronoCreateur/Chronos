...
function clampViewToProject(width = getSize().width) {
  const project = currentProject();
  if (!project) return;
  const { start, end, range } = projectBounds(project);
  const minScale = minScaleForProject(width, project);
  state.view.scale = clamp(state.view.scale, minScale, 40);
  const halfVisible = width / (2 * state.view.scale);
  if (halfVisible * 2 >= range) {
    state.view.center = (start + end) / 2;
  } else {
    state.view.center = clamp(state.view.center, start + halfVisible, end - halfVisible);
  }
